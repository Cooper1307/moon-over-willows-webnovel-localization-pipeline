...
2. **特有文化词汇迭代**：
   ...
   - 更新“小鸡双色莲子羹”为前置形容的 `Two-Color Lotus Seed Chicken Soup`。
   - 第一处出现“衙门”的地方补充了译者注 (TN) 来说明 `local yamen` 的背景。
3. **术语表步进同步**：将修订后确定的终版短语（例如 "an old maid", "yamen", "innocently devoted childhood sweethearts", "mahogany" 等）同步更新至了《第4章术语与人设.md》。
4. **后续优化方针确立**：建立了从直译向母语化过渡的策略，优先采取动态同义词、结构重组以及内嵌式自然注释的发展方向。
...
2. **术语级母语化修正**：
   ...
   - 解决了过于直白的机械翻译，把 `splashing some silver` 精修为了 `spending a few taels of silver` 以符合时代物料背景。
3. **上下文前后一致性校准**：修正了 `Zhu household` 回归为第四章建立的统一设定 `Zhu residence`。
